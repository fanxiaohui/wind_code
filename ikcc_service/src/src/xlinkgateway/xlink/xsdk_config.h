...
extern XLINK_FUNC int XlinkBuildSubKey(void);
...
//#define __XLINK_LOCAL_PORT__ 5988

#define XSDK_DEBUG_DEBUG(info,y...)  do{ if (g_xlink_user_config->DebugPrintf) {g_xlink_user_config->DebugPrintf("XSDK debug=>fun(%s)line %04d:="info,__FUNCTION__,__LINE__,##y);printf("\n");}}while(0)
#define XSDK_DEBUG_WARING(info,y...) do{ if (g_xlink_user_config->DebugPrintf) {g_xlink_user_config->DebugPrintf("XSDK waring=>fun(%s)line %04d:="info,__FUNCTION__,__LINE__,##y);}}while(0)
#define XSDK_DEBUG_ERROR(info,y...)  do{ if (g_xlink_user_config->DebugPrintf) { g_xlink_user_config->DebugPrintf("XSDK error =>fun(%s)line %04d:="info,__FUNCTION__,__LINE__,##y);}}while(0)
