 ...
 */
 static void XlinkMD5Update(XGMD5_CTX *ctx, unsigned char *buf, unsigned int len) {
	...
	if (t) {
		...
		t = 64 - t;
		if (len < t) {
			xlink_memcpy(p, buf, len);
			return;
		}
		xlink_memcpy(p, buf, t);
 ...
 static void XlinkMD5Final(unsigned char *digest, XGMD5_CTX *ctx) {
	...
	*p++ = 0x80;
	...
	if (count < 8) {
		...
		xlink_memset(ctx->in, 0, 56);
	}
	else {
		/* Pad block to 56 bytes */
		xlink_memset(p, 0, count - 8);
	...
	xlink_memcpy(digest, ctx->buf, 16);
	xlink_memset(ctx, 0, sizeof(XGMD5_CTX)); /* In case it's sensitive */
}
